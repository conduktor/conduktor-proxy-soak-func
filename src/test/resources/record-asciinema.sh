...
# Go into each step and record a move out of its results
for stepSh in $(ls step*sh | sort ) ; do
    ...
    asciinema rec \
      ...
      --idle-time-limit 2 \
      --cols 140 --rows $rows \
      --command "source utils.sh; type $step.sh; ./$step.sh" \
      --overwrite \
...
# Extract markers and push them to asciinema.org
markers=""
step=1
